...
/*#######################################################################################
	[概要] OutputStringとは "Matrixの描画範囲の値を指定した文字列で出力する" 機能を持つクラスである。
	[Summary] OutputString is a class that outputs Matrix values as strings.
#######################################################################################*/
		template<typename Matrix_Var_, typename OutputStringName_ = ::std::string>
		class OutputString {
		private:

...
#######################################################################################*/
#ifndef INCLUDED_DUNGEON_TEMPLATE_LIBRARY_DTL_RETOUCH_CELLULAR_AUTOMATON_HPP
#define INCLUDED_DUNGEON_TEMPLATE_LIBRARY_DTL_RETOUCH_CELLULAR_AUTOMATON_HPP

/*#######################################################################################
	日本語リファレンス (Reference-JP)
	https://github.com/Kasugaccho/DungeonTemplateLibrary/wiki/dtl::retouch::CellularAutomaton-(修正クラス)/
#######################################################################################*/
...
namespace dtl {
	inline namespace retouch { //"dtl::retouch"名前空間に属する
		...
		class CellularAutomaton : public ::dtl::range::RectBase<CellularAutomaton<Matrix_Var_>>,
			public ::dtl::utility::DrawJagged<CellularAutomaton<Matrix_Var_>, Matrix_Var_> {
		private:
			...
			using ShapeBase_t = ::dtl::range::RectBase<CellularAutomaton>;
			using DrawBase_t = ::dtl::utility::DrawJagged<CellularAutomaton, Matrix_Var_>;
			...
			DTL_VERSIONING_CPP14_CONSTEXPR
				...
				if (matrix_(col - 1, row) == matrix_(col + 1, row) &&
					matrix_(col + 1, row) == matrix_(col, row - 1) &&
					matrix_(col, row - 1) == matrix_(col, row + 1))
					matrix_(col, row) = matrix_(col + 1, row);
				else switch (DTL_RANDOM_ENGINE.getBit2()) {
				case 0:matrix_(col, row) = matrix_(col - 1, row); break;
				case 1:matrix_(col, row) = matrix_(col + 1, row); break;
				case 2:matrix_(col, row) = matrix_(col, row - 1); break;
				case 3:matrix_(col, row) = matrix_(col, row + 1); break;
				}
			}

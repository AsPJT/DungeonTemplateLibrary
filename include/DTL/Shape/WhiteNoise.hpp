...
namespace dtl {
	inline namespace shape { //"dtl::shape"名前空間に属する
		...
		class WhiteNoise : public ::dtl::range::RectBaseWithValue<WhiteNoise<Matrix_Int_>, Matrix_Int_>,
			public ::dtl::utility::DrawJagged<WhiteNoise<Matrix_Int_>, Matrix_Int_> {
		private:
			...
			using DrawBase_t = ::dtl::utility::DrawJagged<WhiteNoise, Matrix_Int_>;
			...
			DTL_VERSIONING_CPP14_CONSTEXPR
				...
				for (Index_Size row{ this->start_y }; row < end_y_; ++row) {
					...
					for (Index_Size col{ this->start_x }; col < end_x_; ++col)
						if (::dtl::random::mt32bit.probability()) matrix_.set(col, row, this->draw_value, args_...);
				}
				return true;
			}

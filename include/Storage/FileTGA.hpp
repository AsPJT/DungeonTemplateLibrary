...
#######################################################################################*/
#ifndef INCLUDED_DUNGEON_TEMPLATE_LIBRARY_STORAGE_FILE_TGA_HPP
#define INCLUDED_DUNGEON_TEMPLATE_LIBRARY_STORAGE_FILE_TGA_HPP

/*#######################################################################################
	日本語リファレンス (Reference-JP)
	https://github.com/Kasugaccho/DungeonTemplateLibrary/wiki/dtl::storage::FileTGA-(%E3%82%B9%E3%83%88%E3%83%AC%E3%83%BC%E3%82%B8%E3%82%AF%E3%83%A9%E3%82%B9)/
#######################################################################################*/
	...
	inline namespace storage {
		...
		private:
			...
			template<typename Matrix_, typename ...Args_>
				...
				stbi_write_tga(this->str.c_str(), static_cast<int>(end_x_ - this->start_x), static_cast<int>(end_y_ - this->start_y), static_cast<int>(this->color_num), data.get());
				return true;
			}
		...
		public:
			...
			//始点座標Xを初期値に戻す
			DTL_CPP14_CONSTEXPR
				FileTGA& clearPointX() noexcept {
				this->start_x = 0;
				return *this;
			}
			//始点座標Yを初期値に戻す
			DTL_CPP14_CONSTEXPR
				FileTGA& clearPointY() noexcept {
				this->start_y = 0;
				return *this;
			}
			//範囲の大きさ(X軸方向)を初期値に戻す
			DTL_CPP14_CONSTEXPR
				FileTGA& clearWidth() noexcept {
				this->width = 0;
				return *this;
			}
			//範囲の大きさ(Y軸方向)を初期値に戻す
			DTL_CPP14_CONSTEXPR
				FileTGA& clearHeight() noexcept {
				this->height = 0;
				return *this;
			}
			//始点座標(X,Y)を初期値に戻す
			DTL_CPP14_CONSTEXPR
				FileTGA& clearPoint() noexcept {
				this->clearPointX();
				this->clearPointY();
				return *this;
			}
			...
			DTL_CPP14_CONSTEXPR
				FileTGA& setPointX(const Index_Size end_x_) noexcept {
				this->start_x = end_x_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setPointY(const Index_Size end_y_) noexcept {
				this->start_y = end_y_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setWidth(const Index_Size width_) noexcept {
				this->width = width_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setHeight(const Index_Size height_) noexcept {
				this->height = height_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setPoint(const Index_Size point_) noexcept {
				this->start_x = point_;
				this->start_y = point_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setPoint(const Index_Size end_x_, const Index_Size end_y_) noexcept {
				this->start_x = end_x_;
				this->start_y = end_y_;
				return *this;
			}
			DTL_CPP14_CONSTEXPR
				FileTGA& setRange(const Index_Size end_x_, const Index_Size end_y_, const Index_Size length_) noexcept {
				this->start_x = end_x_;
				this->start_y = end_y_;
				this->width = length_;
				this->height = length_;
			...
			DTL_CPP14_CONSTEXPR
				FileTGA& setRange(const Index_Size end_x_, const Index_Size end_y_, const Index_Size width_, const Index_Size height_) noexcept {
				this->start_x = end_x_;
				this->start_y = end_y_;
				this->width = width_;
				this->height = height_;
			...
			DTL_CPP14_CONSTEXPR
				FileTGA& setRange(const dtl::base::MatrixRange & matrix_range_) noexcept {
			...
			constexpr explicit FileTGA(const std::string & write_value_) noexcept
				:str(write_value_) {}
			constexpr explicit FileTGA(const std::string & write_value_, const std::size_t color_num_) noexcept
				:str(write_value_), color_num(color_num_) {}

			constexpr explicit FileTGA(const dtl::base::MatrixRange & matrix_range_) noexcept
				:start_x(matrix_range_.x), start_y(matrix_range_.y),
				width(matrix_range_.w), height(matrix_range_.h) {}
			constexpr explicit FileTGA(const dtl::base::MatrixRange & matrix_range_, const std::string & write_value_) noexcept
				:start_x(matrix_range_.x), start_y(matrix_range_.y),
				width(matrix_range_.w), height(matrix_range_.h),
				str(write_value_) {}
			constexpr explicit FileTGA(const dtl::base::MatrixRange & matrix_range_, const std::string & write_value_, const std::size_t color_num_) noexcept
				:start_x(matrix_range_.x), start_y(matrix_range_.y),
				width(matrix_range_.w), height(matrix_range_.h),
				str(write_value_), color_num(color_num_) {}

			constexpr explicit FileTGA(const Index_Size end_x_, const Index_Size end_y_, const Index_Size width_, const Index_Size height_) noexcept
				:start_x(end_x_), start_y(end_y_),
				width(width_), height(height_) {}
			constexpr explicit FileTGA(const Index_Size end_x_, const Index_Size end_y_, const Index_Size width_, const Index_Size height_, const std::string & write_value_) noexcept
				:start_x(end_x_), start_y(end_y_),
				width(width_), height(height_),
				str(write_value_) {}
			constexpr explicit FileTGA(const Index_Size end_x_, const Index_Size end_y_, const Index_Size width_, const Index_Size height_, const std::string & write_value_, const std::size_t color_num_) noexcept
				:start_x(end_x_), start_y(end_y_),
				width(width_), height(height_),
				str(write_value_), color_num(color_num_) {}
		};

...
/*#######################################################################################
	...
	https://github.com/Kasugaccho/DungeonTemplateLibrary/wiki/dtl::shape::MixRect-(%E5%BD%A2%E7%8A%B6%E3%82%AF%E3%83%A9%E3%82%B9)/
#######################################################################################*/
	...
	inline namespace shape {
		...
		private:
			...
			constexpr inline void substitutionSTL(Matrix_&& matrix_, const Index_Size point_x_, const Index_Size point_y_) const noexcept {
				matrix_[point_y_][point_x_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
			template<typename Matrix_>
			constexpr inline void substitutionArray(Matrix_&& matrix_, const Index_Size point_x_, const Index_Size point_y_, const Index_Size max_x_) const noexcept {
				matrix_[point_y_ * max_x_ + point_x_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
			template<typename Matrix_>
			constexpr inline void substitutionLayer(Matrix_&& matrix_, const Index_Size layer_, const Index_Size point_x_, const Index_Size point_y_) const noexcept {
				matrix_[point_y_][point_x_][layer_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
			...
			constexpr inline void substitutionSTL(Matrix_&& matrix_, const Index_Size point_x_, const Index_Size point_y_, Function_&& function_) const noexcept {
				if (function_(matrix_[point_y_][point_x_])) matrix_[point_y_][point_x_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
			template<typename Matrix_, typename Function_>
			constexpr inline void substitutionArray(Matrix_&& matrix_, const Index_Size point_x_, const Index_Size point_y_, const Index_Size max_x_, Function_&& function_) const noexcept {
				if (function_(matrix_[point_y_ * max_x_ + point_x_])) matrix_[point_y_ * max_x_ + point_x_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
			template<typename Matrix_, typename Function_>
			constexpr inline void substitutionLayer(Matrix_ && matrix_, const Index_Size layer_, const Index_Size point_x_, const Index_Size point_y_, Function_ && function_) const noexcept {
				if (function_(matrix_[point_y_][point_x_][layer_])) matrix_[point_y_][point_x_][layer_] = this->draw_value[dtl::random::mt32bit.get<std::size_t>(draw_value.size())];
			}
		...
		public:
			...
			//始点座標Xを初期値に戻す
			constexpr MixRect& clearPointX() noexcept {
				this->point_x = 0;
				return *this;
			}
			//始点座標Yを初期値に戻す
			constexpr MixRect& clearPointY() noexcept {
				this->point_y = 0;
				return *this;
			}
			//範囲の大きさ(X軸方向)を初期値に戻す
			constexpr MixRect& clearWidth() noexcept {
				this->width = 0;
				return *this;
			}
			//範囲の大きさ(Y軸方向)を初期値に戻す
			constexpr MixRect& clearHeight() noexcept {
				this->height = 0;
				return *this;
			}
			//塗り値を初期値に戻す
			constexpr MixRect& clearValue() noexcept {
				std::vector<Matrix_Int_> new_draw_value{};
				this->draw_value = std::move(new_draw_value);
				return *this;
			}
			//始点座標(X,Y)を初期値に戻す
			constexpr MixRect& clearPoint() noexcept {
				this->clearPointX();
				this->clearPointY();
				return *this;
			}
			//描画範囲を初期値に戻す
			constexpr MixRect& clearRange() noexcept {
				this->clearPointX();
				this->clearPointY();
				this->clearWidth();
				this->clearHeight();
			...
			//全ての値を初期値に戻す
			constexpr MixRect& clear() noexcept {
				this->clearRange();
				this->clearValue();
				return *this;
			}
			...
			constexpr MixRect& setPointX(const Index_Size point_x_) noexcept {
				this->point_x = point_x_;
				return *this;
			}
			constexpr MixRect& setPointY(const Index_Size point_y_) noexcept {
				this->point_y = point_y_;
				return *this;
			}
			constexpr MixRect& setWidth(const Index_Size width_) noexcept {
				this->width = width_;
				return *this;
			}
			constexpr MixRect& setHeight(const Index_Size height_) noexcept {
				this->height = height_;
				return *this;
			}
			constexpr MixRect& setValue(const std::vector<Matrix_Int_>& draw_value_) noexcept {
				this->draw_value = draw_value_;
				return *this;
			}
			template<typename ...Args_>
			constexpr MixRect& setValue(const Matrix_Int_& first_, const Args_& ... args_) noexcept {
				this->string_String(first_, args_...);
				return *this;
			}
			constexpr MixRect& setPoint(const Index_Size point_) noexcept {
				this->point_x = point_;
				this->point_y = point_;
				return *this;
			}
			constexpr MixRect& setPoint(const Index_Size point_x_, const Index_Size point_y_) noexcept {
				this->point_x = point_x_;
				this->point_y = point_y_;
				return *this;
			}
			constexpr MixRect& setRange(const Index_Size point_x_, const Index_Size point_y_, const Index_Size length_) noexcept {
				this->point_x = point_x_;
				this->point_y = point_y_;
				this->width = length_;
				this->height = length_;
			...
			constexpr MixRect& setRange(const Index_Size point_x_, const Index_Size point_y_, const Index_Size width_, const Index_Size height_) noexcept {
				this->point_x = point_x_;
				this->point_y = point_y_;
				this->width = width_;
				this->height = height_;
			...
			constexpr MixRect& setRange(const dtl::base::MatrixRange& matrix_range_) noexcept {

...
namespace dtl {
	namespace file {
		...
		namespace read {
			...
			Matrix_Int_ splitReturnValue_csv(const std::string& field_) noexcept {
				return static_cast<Matrix_Int_>(std::stoi(field_));
			}
			template<>
			long splitReturnValue_csv<long>(const std::string& field_) noexcept {
				return std::stol(field_);
			}
			template<>
			long long splitReturnValue_csv<long long>(const std::string& field_) noexcept {
				return std::stoll(field_);
			}
			template<>
			unsigned long splitReturnValue_csv<unsigned long>(const std::string& field_) noexcept {
				return std::stoul(field_);
			}
			template<>
			unsigned long long splitReturnValue_csv<unsigned long long>(const std::string& field_) noexcept {
				return std::stoull(field_);
			}
			template<>
			float splitReturnValue_csv<float>(const std::string& field_) noexcept {
				return std::stof(field_);
			}
			template<>
			double splitReturnValue_csv<double>(const std::string& field_) noexcept {
				return std::stod(field_);
			}
			template<>
			long double splitReturnValue_csv<long double>(const std::string& field_) noexcept {
			...
			void split_csv(Matrix_& matrix_, const std::size_t y_id_, std::string& input_line_, const char delimiter_) noexcept {
				...
				while (std::getline(stream, field, delimiter_)) {
					...
					matrix_[y_id_][x_id] = dtl::file::read::splitReturnValue_csv<Matrix_Int_>(field);
					++x_id;
				}
				return;
			}
			...
			bool csv(Matrix_& matrix_, const std::string& str_, const char delimiter_ = ',') noexcept {
				...
				while (std::getline(ifs, line)) {
					dtl::file::read::split_csv<Matrix_Int_>(matrix_, y_id, line, delimiter_);
					++y_id;
				}
				return true;
			}
		...
		namespace write {
			...
			bool svgColor(const Matrix_& matrix_, const std::string& str_, const std::size_t color_true_ = 0, const std::size_t color_false_ = 0xffffff) noexcept {
				...
				dtl::file::write::stringTemplate_svg(matrix_, ofs);
				...
				for (std::size_t row{}; row < matrix_.size(); ++row) {
					if (matrix_[row].size() == 0) continue;
					if (matrix_[row][0]) rectTemplateColor_svg(ofs, 0, row);
					for (std::size_t col{ 1 }; col < matrix_[row].size(); ++col) {
						if (matrix_[row][col]) rectTemplateColor_svg(ofs, col, row);
					}
				}
				ofs << "</svg>";
				return true;
			...
			bool svg(const Matrix_& matrix_, const std::string& str_) noexcept {
				...
				stringTemplate_svg(matrix_, ofs);

				for (std::size_t row{}; row < matrix_.size(); ++row) {
					if (matrix_[row].size() == 0) continue;
					if (matrix_[row][0]) rectTemplate_svg(ofs, 0, row);
					for (std::size_t col{ 1 }; col < matrix_[row].size(); ++col) {
						if (matrix_[row][col]) rectTemplate_svg(ofs, col, row);
					}
				}
				ofs << "</svg>";
				return true;
			...
			//objファイルの書き込み
			template<typename Matrix_>
			bool writeTerrain_obj(const Matrix_& matrix_, const std::string& str_) noexcept {
				if (matrix_.size() == 0 || matrix_[0].size() == 0) return false;
				std::ofstream ofs(str_);
				if (ofs.fail()) return false;
			...
			void write_objOutputCube(std::ofstream& ofs_, std::int_fast32_t start_x, std::int_fast32_t start_y, std::int_fast32_t start_z, std::int_fast32_t size_x, std::int_fast32_t size_y, std::int_fast32_t size_z, std::size_t id_ = 0) noexcept {
				...
				write_objOutputId(ofs_, id_);
			}

			template<typename Matrix_Int_, typename Matrix_>
			bool writeBoard_obj(const Matrix_& matrix_, const std::string& str_, const std::int_fast32_t block_size_ = 1, const Matrix_Int_ output_id_ = 1) noexcept {
				if (matrix_.size() == 0 || matrix_[0].size() == 0) return false;
				std::ofstream ofs(str_);
				if (ofs.fail()) return false;

				write_objOutputCube(ofs, 0, -1, 0, (std::int_fast32_t)matrix_[0].size(), 1, (std::int_fast32_t)matrix_.size());

				std::size_t square_count{ 1 };
				for (std::size_t row{}; row < matrix_.size(); ++row)
					for (std::size_t col{}; col < matrix_[row].size(); ++col)
						if (matrix_[row][col] == output_id_) {
							write_objOutputCube(ofs, (std::int_fast32_t)col, 0, (std::int_fast32_t)row, 1, block_size_, 1, square_count);
							++square_count;
						}
				return true;
			}
			...
			void writeMatrixSize_bmp(std::array<std::uint_fast8_t, file_write_bmp_format_1bit_size>& bmp_file_, std::uint32_t x_, std::uint32_t y_) noexcept {
			...
			bool bmp(const Matrix_& matrix_, const std::string& str_, const std::uint32_t x_, const std::uint32_t y_) noexcept {
				...
				writeMatrixSize_bmp(format_1bit, x_, y_);
			...
			bool all(const Matrix_& matrix_, const std::string& str_) noexcept {
				//Text
				dtl::file::write::dtlm(matrix_, str_ + ".dtlm");
				dtl::file::write::csv(matrix_, str_ + ".csv");
				dtl::file::write::md(matrix_, str_ + ".md");
				dtl::file::write::txt_0_9(matrix_, str_ + "_0_9.txt");
				//Picture
				dtl::file::write::pbm(matrix_, str_ + ".pbm");
				dtl::file::write::svg(matrix_, str_ + ".svg");
				dtl::file::write::bmp(matrix_, str_ + ".bmp");
				return true;
			}
			template<typename Matrix_>
			bool folderAll(const Matrix_& matrix_, const std::string& str_1, const std::string& str_2) noexcept {
				//Text
				dtl::file::write::dtlm(matrix_, str_1 + "dtlm\\" + str_2 + ".dtlm");
				dtl::file::write::csv(matrix_, str_1 + "csv\\" + str_2 + ".csv");
				dtl::file::write::md(matrix_, str_1 + "md\\" + str_2 + ".md");
				dtl::file::write::txt_0_9(matrix_, str_1 + "txt\\" + str_2 + "_0_9.txt");
				//Picture
				dtl::file::write::pbm(matrix_, str_1 + "pbm\\" + str_2 + ".pbm");
				dtl::file::write::svg(matrix_, str_1 + "svg\\" + str_2 + ".svg");
				dtl::file::write::bmp(matrix_, str_1 + "bmp\\" + str_2 + ".bmp");
				return true;
			}

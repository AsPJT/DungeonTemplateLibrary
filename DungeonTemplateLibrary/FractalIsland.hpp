...
namespace dtl {
		...
		constexpr void create(STL_& world_map, std::size_t seed_ = 0, const std::int_fast32_t max_value_ = 255) const {
			...
			for (std::size_t i{}; i < chunk_x; ++i)
				for (std::size_t j{}; j < chunk_y; ++j) {
					...
					rnd.seed((std::uint_fast32_t)(seed_ + ((i + 1) % chunk_x) + (j * aslib_world_make_seed1) + ((((i + 1) % chunk_x) ^ j) * aslib_world_make_seed2)));
					map_[16][0] = (Int_)rnd(max_value_);
					rnd.seed((std::uint_fast32_t)(seed_ + i + (((j + 1) % chunk_y) * aslib_world_make_seed1) + ((i ^ ((j + 1) % chunk_y)) * aslib_world_make_seed2)));
					map_[0][16] = (Int_)rnd(max_value_);
					rnd.seed((std::uint_fast32_t)(seed_ + ((i + 1) % chunk_x) + (((j + 1) % chunk_y) * aslib_world_make_seed1) + ((((i + 1) % chunk_x) ^ ((j + 1) % chunk_y)) * aslib_world_make_seed2)));
	...
	private:
		...
		constexpr void worldMapMake(const std::size_t x_, const std::size_t y_, const std::size_t size_, const std::size_t t1_, const std::size_t t2_, const std::size_t t3_, const std::size_t t4_, std::array<std::array<Int_, 17>, 17>& map_, const std::size_t max_value_)const {
			...
			if (size_ == 0) return;
			//頂点の高さを決める
			const std::size_t vertex_height{ ((t1_ + t2_ + t3_ + t4_) / 4) + (std::size_t)rnd((std::int_fast32_t)size_) };
			...
			//4つの地点の座標を決める
			map_[x_ + size_][y_] = (Int_)s3;
			map_[x_ - size_][y_] = (Int_)s2;
			map_[x_][y_ + size_] = (Int_)s4;
			map_[x_][y_ - size_] = (Int_)s1;

			//分割サイズを半分にする
			const std::size_t& size{ size_ / (std::size_t)2 };
			//4つに分割

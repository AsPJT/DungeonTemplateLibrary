...
namespace dtl {
	...
	constexpr void createDiamondSquareAverage_Map(const std::size_t x_, const std::size_t y_, std::size_t size_, const Matrix_Int_ t1_, const Matrix_Int_ t2_, const Matrix_Int_ t3_, const Matrix_Int_ t4_, Matrix_& matrix_, const Matrix_Int_ max_value_) noexcept {
		...
		//頂点の高さを決める
		const Matrix_Int_ vertex_height{ static_cast<Matrix_Int_>(((t1_ + t2_ + t3_ + t4_) / 4) + rnd(static_cast<std::int_fast32_t>(size_))) };
		matrix_[y_][x_] = ((vertex_height > max_value_) ? max_value_ : vertex_height);
		//四角形の2点同士の中点の高さを決定
	...
	public:
		...
		constexpr explicit SimpleDiamondSquareAverageIsland(Matrix_& matrix_, const Matrix_Int_ max_value_ = 31) noexcept {
			create(matrix_, max_value_);
		}
		//ワールドマップ生成
		template<typename Matrix_>
		constexpr void create(Matrix_& matrix_, const Matrix_Int_ max_value_ = 31) const noexcept {
	...
	public:
		...
		template<typename Matrix_>
		constexpr explicit FractalIsland(Matrix_& matrix_, std::size_t seed_ = 0, const std::int_fast32_t max_value_ = 31) noexcept {
			create(matrix_, seed_, max_value_);
		}
		//ワールドマップ生成
		template<typename Matrix_>
		constexpr void create(Matrix_& matrix_, std::size_t seed_ = 0, const std::int_fast32_t max_value_ = 31) const noexcept {
			if (matrix_.size() == 0 || matrix_[0].size() == 0) return;

			if (seed_ == 0) seed_ = static_cast<std::size_t>(rnd(0xffff));
			std::array<std::array<Matrix_Int_, 17>, 17> chunk_matrix{ {} };

			const std::size_t chunk_x{ (matrix_[0].size() / 16) };
			const std::size_t chunk_y{ (matrix_.size() / 16) };
			...
			for (std::size_t row{}; row < chunk_y; ++row)
				for (std::size_t col{}; col < chunk_x; ++col) {
					//四角形の4点の高さを決定
					rnd.seed(static_cast<std::uint_fast32_t>(seed_ + row + (col * aslib_world_make_seed1) + ((row^col) * aslib_world_make_seed2)));
					chunk_matrix[0][0] = static_cast<Matrix_Int_>(rnd(max_value_));
					rnd.seed(static_cast<std::uint_fast32_t>(seed_ + ((row + 1) % chunk_y) + (col * aslib_world_make_seed1) + ((((row + 1) % chunk_y) ^ col) * aslib_world_make_seed2)));
					chunk_matrix[16][0] = static_cast<Matrix_Int_>(rnd(max_value_));
					rnd.seed(static_cast<std::uint_fast32_t>(seed_ + row + (((col + 1) % chunk_x) * aslib_world_make_seed1) + ((row ^ ((col + 1) % chunk_x)) * aslib_world_make_seed2)));
					chunk_matrix[0][16] = static_cast<Matrix_Int_>(rnd(max_value_));
					rnd.seed(static_cast<std::uint_fast32_t>(seed_ + ((row + 1) % chunk_y) + (((col + 1) % chunk_x) * aslib_world_make_seed1) + ((((row + 1) % chunk_y) ^ ((col + 1) % chunk_x)) * aslib_world_make_seed2)));
					chunk_matrix[16][16] = static_cast<Matrix_Int_>(rnd(max_value_));

					//チャンク生成
					createWorldMapSimple(chunk_matrix, static_cast<Matrix_Int_>(max_value_));
					//生成したチャンクをワールドマップにコピペ
					for (std::size_t row2{}; row2 < 16; ++row2)
						for (std::size_t col2{}; col2 < 16; ++col2)
							matrix_[row * 16 + row2][col * 16 + col2] = chunk_matrix[row2][col2];
				}
		}
	private:
		//チャンク生成の呼び出し・実行
		constexpr void createWorldMapSimple(std::array<std::array<Matrix_Int_, 17>, 17>& matrix_, const Matrix_Int_ max_value_) const noexcept {
			createDiamondSquareAverage_Map<Matrix_Int_>(8, 8, 8, matrix_[0][0], matrix_[16][0], matrix_[0][16], matrix_[16][16], matrix_, max_value_);
		}
	};
}

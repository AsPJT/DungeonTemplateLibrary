...
namespace dtl::generator::common::layer::stl {
	...
	//全てのマスを0で埋める
	template<typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_) noexcept {
		for (std::size_t row{}; row < matrix_.size(); ++row)
			for (std::size_t col{}; col < matrix_[row].size(); ++col)
	...
	//全てのマスを指定した数値で埋める
	template<typename Matrix_Int_, typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_, const Matrix_Int_ value_) noexcept {
		for (std::size_t row{}; row < matrix_.size(); ++row)
			for (std::size_t col{}; col < matrix_[row].size(); ++col)
	...
	public:
		...
		template<typename Matrix_>
		constexpr void create(Matrix_& matrix_, const std::size_t layer_, const Matrix_Int_ value_ = 0) const noexcept {
...
namespace dtl::generator::common::layer::normal {
	...
	//全てのマスを0で埋める
	template<typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_, const std::size_t x_, const std::size_t y_) noexcept {
		for (std::size_t row{}; row < y_; ++row)
			for (std::size_t col{}; col < x_; ++col)
				matrix_[row][col][layer_] = 0;
	}
	//全てのマスを指定した数値で埋める
	template<typename Matrix_Int_, typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_, const std::size_t x_, const std::size_t y_, const Matrix_Int_ value_) noexcept {
		for (std::size_t row{}; row < y_; ++row)
			for (std::size_t col{}; col < x_; ++col)
				matrix_[row][col][layer_] = value_;
	}
	template<typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_, const std::size_t start_x_, const std::size_t start_y_, const std::size_t end_x_, const std::size_t end_y_) noexcept {
		for (std::size_t row{ start_y_ }; row < end_y_; ++row)
			for (std::size_t col{ start_x_ }; col < end_x_; ++col)
				matrix_[row][col][layer_] = 0;
	}
	template<typename Matrix_Int_, typename Matrix_>
	constexpr void createDungeonInit(Matrix_& matrix_, const std::size_t layer_, const std::size_t start_x_, const std::size_t start_y_, const std::size_t end_x_, const std::size_t end_y_, const Matrix_Int_ value_) noexcept {
		for (std::size_t row{ start_y_ }; row < end_y_; ++row)
			for (std::size_t col{ start_x_ }; col < end_x_; ++col)
				matrix_[row][col][layer_] = value_;
	}
	...
	public:
		...
		template<typename Matrix_>
		constexpr void create(Matrix_& matrix_, const std::size_t layer_, const std::size_t x_, const std::size_t y_, const Matrix_Int_ value_ = 0) const noexcept {
